...
MAX_SIZE = 256e4  # change here accroading to your GPU memory
...
def scale_resize(image, target=1600):
    """等比缩放"""
...
def extract_data(archive: ZipFile, dir_name: str, image_path:str) -> Data:
    ...
    with open(f"{image_path}/{dir_name}.txt", 'r', encoding='utf-8') as f:
        captions = f.readlines()
    data = []
    for line in tqdm.tqdm(captions):
        ...
        if len(formula) > 1050:
            continue
        img = Image.open(f"{image_path}/{img_name}").copy()
...
def build_dataset(archive, folder: str, batch_size: int, image_path:str):
    data = extract_data(archive, folder, image_path)
...
class CROHMEDatamodule(pl.LightningDataModule):
    def __init__(
        ...
        image_path: str = "/hy-tmp/data/icpr_data",
        test_year: str = "2014",
        ...
        with ZipFile(self.zipfile_path) as archive:
            if stage == "fit" or stage is None:
                self.train_dataset = CROHMEDataset(
                    build_dataset(archive, "train", self.train_batch_size, self.image_path),
                    True,
                    self.scale_aug,
                )
                self.val_dataset = CROHMEDataset(
                    build_dataset(archive, 'val', self.eval_batch_size, self.image_path),
                    False,
                    self.scale_aug,
                )
