...
from utils.metrics import AverageMeter, TokenAccMetric
from utils import logger
from utils.dl_util import get_optimizer
from utils.utils import get_absolute_file_path, merge_config
from transformers import AutoTokenizer, VisionEncoderDecoderModel, VisionEncoderDecoderConfig, NougatProcessor
from dataset.processor.image_processing_nougat import NougatImageProcessor
from dataset.processor.image_processor_img2latex  import NougatLaTexProcessor
...
class DonutExperiment(BaseExperiment):
        ...
        for i, batch_data in tqdm(enumerate(self.eval_data_loader), total=len(self.eval_data_loader)):
            ...
            with torch.no_grad():
                outputs = self.model.module.generate(
                    ...
                    max_length=self.model.module.decoder.config.max_length,
    ...
    def _get_data_loader_from_dataset(self, dataset, data_loader_args, phase="train"):
        ...
        if collate_fn_args.get("type") is None:
            ...
            collate_fn = getattr(mydataset, collate_fn_type)(batch_size=batch_size, processor=self.processor,
                                                              **collate_fn_args)
