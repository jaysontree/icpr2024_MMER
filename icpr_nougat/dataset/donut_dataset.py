...
from dataset.processor.util import process_raw_latex_code
from torch.nn.utils.rnn import pad_sequence
from torch.utils.data import Dataset
from torchvision.transforms.functional import to_pil_image
from tqdm import tqdm
from dataset.augment import DataAug
...
class NougatDataset(Dataset):
    def __init__(
        ...
        self.data_root = data_root[0]
        with open(os.path.join(self.data_root, equations), 'r', encoding='utf-8') as f:
            labels = f.readlines()
        ...
        for lab in labels:
            ...
            if not os.path.exists(os.path.join(self.data_root, name)):
                continue
            self.labels.append(lab)

        self.pairs = list()
        for i, lab in tqdm(enumerate(self.labels), total=len(self.labels)):
            name, labels = self.labels[i].strip().split("\t")
            text = labels.strip()
            im = os.path.join(self.data_root, name)
            self.pairs.append((text, im))
        if phase == "train":
    ...
    def __getitem__(self, idx: int):
        ...
        input_ids, attention_mask = encode_text(eqs.split(' '), max_target_length=self.max_length, vocab=self.vocab)
        tok = {'input_ids': input_ids,
        ...
        if self.phase == "train":
            ...
            if np.random.random() < .04:
                im[im != 255] = 0
        if self.transforms is not None and self.phase == "train":
...
class NougatPadFixSizeCollectFn(object):
    def __init__(self, batch_size, processor, debug=False, maxH=None, **kwargs):
        ...
        self.maxH = self.imgH if maxH is None else maxH
