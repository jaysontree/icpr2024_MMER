...
class TIAPerspective(TransBase):
	...
	def trans_function(self, _image):
		...
		img_h, img_w = image.shape[:2]
		if img_h >= 20 and img_w >= 20:
			thresh = min(img_w, img_h) // self.segment
...
class DataAug:
	def __init__(self):
		...
		# 随机颜色增强
		self.random_color = RandomColor(probability=0.3)
		# 随机锐化
		self.random_sharpness = RandomSharpness(probability=0.3)
		# 随机压缩率，利用jpeg的有损压缩来增广
		self.compress = Compress(probability=0.3)
		# 随机区域曝光
		self.exposure = Exposure(probability=0.3)
		# 随机旋转
		self.rotate = Rotate(probability=0.3)
		# 随机模糊
		...
		# 随机TIA变形
		self.tia_distort = TIADistort(probability=0.3)  # 宽度方向
		self.tia_stretch = TIAStretch(probability=0.3)  # 长度方向
		self.tia_perspective = TIAPerspective(probability=0.3)  # 多方向

		# 随机颜色扰动
		self.color_disturbance = ColorDisturbance(probability=0.3)
		...
		self.rotate.setparam(fillcolor= (255,255,255))
		self.blur.setparam()
