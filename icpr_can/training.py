...
def train(params, model, optimizer, epoch, train_loader, writer=None):
    ...
    with tqdm(train_loader, total=len(train_loader)//params['train_parts']) as pbar:
        for batch_idx, (images, image_masks, labels, label_masks) in enumerate(pbar):
            ...
            optimizer.zero_grad()
            probs, counting_preds, word_loss, counting_loss,focal_loss = model(images, image_masks, labels, label_masks)
            word_loss = word_loss.mean()
            counting_loss = counting_loss.mean()
            focal_loss = focal_loss.mean()
            loss = word_loss + counting_loss + focal_loss
            loss.backward()
            ...
            if writer:
                ...
                writer.add_scalar('train/focal_loss', focal_loss.item(), current_step)
            ...
            pbar.set_description(f'{epoch+1} word_loss:{word_loss.item():.4f} focal_loss:{focal_loss.item():.4f} counting_loss:{counting_loss.item():.4f} '
    ...
    with tqdm(eval_loader, total=len(eval_loader)//params['valid_parts']) as pbar, torch.no_grad():
        for batch_idx, (images, image_masks, labels, label_masks) in enumerate(pbar):
            images, image_masks, labels, label_masks = images.to(device), image_masks.to(
                device), labels.to(device), label_masks.to(device)
            batch, time = labels.shape[:2]
            probs, counting_preds, word_loss, counting_loss, focal_loss = model(images, image_masks, labels, label_masks, is_train=False)
            word_loss = word_loss.mean()
            counting_loss = counting_loss.mean()
            focal_loss = focal_loss.mean()
            loss = word_loss + counting_loss + focal_loss
            ...
            if writer:
                ...
                writer.add_scalar('eval/focal_loss', focal_loss.item(), current_step)
