...
class CAN(nn.Module):
    ...
    def forward(self, images, images_mask, labels, labels_mask, is_train=True):
        ...
        word_probs, word_alphas, feature = self.decoder(cnn_features, labels, counting_preds, images_mask, labels_mask, is_train=is_train)
        ...
        focal_loss= self.focal_loss(word_probs.contiguous().view(-1, word_probs.shape[-1]), labels.contiguous().view(-1))
        focal_loss = (focal_loss * labels_mask.view(-1)).sum() / (labels_mask.sum() + 1e-10) if self.use_label_mask else focal_loss
        
        return word_probs, counting_preds, word_average_loss, counting_loss, focal_loss


if __name__ == "__main__":
    from utils import load_config
    params = load_config("/home/qiujin/project/CAN_src/config.yaml")
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    params['device'] = device
    params['word_num'] = 458
    
    can = CAN(params=params)