...
def load_checkpoint(model, optimizer, path,skip_missmatch=False):
    ...
    for k, v in model_params.items():
        # tmp_k = "module."+k
        tmp_k = k
        if tmp_k in checkpoint.keys():
            v_checkpoint = checkpoint[tmp_k]
            # print(v_checkpoint.shape,v.shape)
            if v_checkpoint.shape == v.shape:
                new_state_dict[k] = checkpoint[tmp_k]
            else:
                print('mismatched parameters ', k)
                if skip_missmatch:
                    new_state_dict[k] = v
                else:
                    raise Exception("Mismatched parameters ", k)
        else:
